...
if [[ ! -r "$KERNEL" || ! -r "$OVERLAY_KO_XZ" ]]; then
    echo "qemu-switch-root-smoke: missing kernel or overlay module" >&2
    exit 1
fi
...
mkdir -p "$ROOT/store"/{base/{sbin,dev,proc,run,sys},override,data,work}
...
cp target/x86_64-unknown-linux-musl/release/examples/switched_root_init "$ROOT/store/base/sbin/init"
xz -dc "$OVERLAY_KO_XZ" > "$ROOT/overlay.ko"
printf 'base\n' > "$ROOT/store/base/handoff-marker"
printf 'override\n' > "$ROOT/store/override/handoff-marker"

(cd "$ROOT" && find . | cpio -o -H newc 2>/dev/null | gzip -9) > "$WORK/initramfs.cpio.gz"
ARGS=(
    -kernel "$KERNEL"
    -initrd "$WORK/initramfs.cpio.gz"
    -append "console=ttyS0 rdinit=/init layerfs.checkpoint=normal"
    -nographic
    -serial mon:stdio
    -no-reboot
    -m 256M
...
if grep -q "QEMU-SWITCH-ROOT: PASS" <<<"$OUTPUT"; then
    echo "qemu-switch-root-smoke: PASS"
else
    echo "qemu-switch-root-smoke: FAIL" >&2
    exit 1
fi

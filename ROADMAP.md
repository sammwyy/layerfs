...
- [~] `main.rs` discovers an already-mounted store by scanning `/proc/self/mountinfo` for a mount containing `base`; `layerfs.store=<path>` accepts an explicit mounted path, while `layerfs.store=/dev/...` mounts an unencrypted Btrfs store at `/run/layerfs-store`. `scripts/qemu-btrfs-store-smoke.sh` creates a loop-mounted Btrfs image in a privileged container, presents it to QEMU as `/dev/vda`, and verifies that the actual initramfs mounts, composes, and switches to it. `layerfs.store=UUID=<uuid>` / `LABEL=<label>` / `PARTUUID=<uuid>` are now resolved to a real device path through the standard `/dev/disk/by-*` udev symlinks before mounting — the same convention `root=UUID=...` uses — so GRUB/systemd-boot-generated cmdlines don't have to hardcode a device name that can change across boots or hardware. Verified for real against this machine's actual disk layout (`/dev/disk/by-uuid` symlinks), not just a fake fixture, plus unit tests for `LABEL=`/`PARTUUID=`, pass-through of an already-literal path, and a clear error for an unresolvable UUID; re-ran `scripts/qemu-btrfs-store-smoke.sh` (still a literal `/dev/vda`) to confirm no regression in the existing path. `layerfs.subvol=<name>` mounts a specific Btrfs subvolume instead of the device's default one, matching section 33's migrated layout where LayerFS lives alongside other subvolumes (`@root`, `@home`, ...) rather than owning the whole filesystem — verified for real (`scripts/btrfs-subvol-smoke.sh`) against a loop-mounted Btrfs image with a real, non-default `layerfs` subvolume in a privileged container: the same device *without* `--subvol` correctly fails (`is not a LayerFS Btrfs store`, since `base` only exists inside the named subvolume, not the default one), and *with* `--subvol layerfs` it's found. LUKS activation remains dracut/migration work.
...
- [~] Fedora dracut module: `integrations/dracut/module-setup.sh` embeds `layerfs-init`, includes OverlayFS, and emits `rdinit=/sbin/layerfs-init`; `scripts/dracut-smoke.sh` builds an isolated image with the real dracut binary and confirms both payloads are present. The module now also installs a `cmdline` hook (`integrations/dracut/parse-layerfs.sh`) that makes the initqueue actually wait for `layerfs.store=`'s backing device before `layerfs-init` ever runs — the exact same mechanism dracut's own `95rootfs-block/parse-block.sh` uses for `root=` (`label_uuid_to_dev` to resolve `UUID=`/`LABEL=`/`PARTUUID=` to a `/dev/disk/by-*` path, then `wait_for_dev` to block the initqueue until udev has actually created it), rather than something bespoke. Without this, `layerfs-init` could race ahead of udev and fail with a spurious "no such device" on a boot where enumeration is slow. Verified for real: built with the actual dracut binary (`scripts/dracut-smoke.sh`, extended to also check the hook lands at `hooks/cmdline/30-parse-layerfs.sh` in the built image) and `sh -n`-checked for syntax errors. Not yet verified with an actual delayed-device QEMU boot proving the wait itself fires (would need a way to make udev enumeration artificially slow inside QEMU); LUKS activation before this point is still not handled — `layerfs-init` still only knows how to mount a plain Btrfs device, not decrypt one first.

...
- [x] `rebuild updates`: every committed transaction already carries the full program+args it ran (`TransactionRecord`, already tracked in-memory); it's now also appended as one JSON line to `manifest.log` on commit, giving a durable package manifest for free instead of a new subsystem. `rebuild updates` discards the current UPDATE/UPDATE_HEAD and replays each manifest entry as a fresh transaction against BASE, in order — `Transaction::skip_manifest()` keeps the replay from re-logging itself. Exact reconstruction still depends on package availability (a `dnf install` replayed later may resolve different versions if repos changed) — an inherent limit of replay, not something this pretends to solve. `doctor` continues to report read-only store health: layer discovery, backend validation, generation consistency, structural BASE checks, and registered boot-artifact completeness.
- [x] verified for real (`unshare --map-root-user --mount --user`): two chained transactions produce a two-line `manifest.log`; `rebuild updates` replaces both UPDATE and UPDATE_HEAD with entirely new generations (confirmed by their paths changing) holding equivalent content (same files landing in the same layers as the original run), the superseded generations are garbage-collected (only the 2 current ones remain), BASE stays untouched, and the manifest itself isn't duplicated by the replay. Also confirmed `rebuild bogus` is rejected and `rebuild updates` against an empty manifest is a harmless no-op.

...
- [~] `main.rs` discovers an already-mounted store by scanning `/proc/self/mountinfo` for a mount containing `base`; `layerfs.store=<path>` accepts an explicit mounted path, while `layerfs.store=/dev/...` mounts an unencrypted Btrfs store at `/run/layerfs-store`. The device path was verified against a loop-mounted Btrfs image in a privileged container. Automatic device discovery, LUKS activation, and Btrfs subvolume selection remain dracut/migration work.

...
- [~] Fedora dracut module: `integrations/dracut/module-setup.sh` embeds `layerfs-init`, includes OverlayFS, and emits `rdinit=/sbin/layerfs-init`; `scripts/dracut-smoke.sh` builds an isolated image with the real dracut binary and confirms both payloads are present. `layerfs-init` can mount an explicit Btrfs device, but the module does not yet ensure that its backing device is available, so this is not a complete boot integration.
...
- [x] `layerctl install --integrations dnf,apt` activates the matching adapter binaries: symlinks each present real binary (`usr/bin/dnf`, `usr/bin/apt-get`/`usr/bin/apt`, `usr/bin/pacman`) to `layerfs-<name>` inside the new base — the GRUB side (baking `layerfs.integrations=` into the cmdline, Milestone 3) was already in place, this was the missing activation step. An unrecognized integration name is rejected at install time rather than silently booting with it inactive; a real binary absent from the source tree (package not installed) is skipped rather than an error
- [x] verified for real: installing a fixture with `usr/bin/dnf` and `usr/bin/pacman` present, `--integrations dnf,pacman` correctly symlinks both to their adapters; `--integrations bogus` is rejected with `unknown integration: bogus`
...
- [~] initramfs-tools integration: `integrations/initramfs-tools/hooks/layerfs` embeds `layerfs-init` plus OverlayFS and Btrfs modules; `scripts/initramfs-tools-smoke.sh` builds a real Debian initramfs from extracted kernel modules and verifies all three payloads. The boot loader must still provide `rdinit=/sbin/layerfs-init` and make `layerfs.store=/dev/...` available. mkinitcpio and systemd-boot remain unstarted.
...
- [ ] not scoped: within a boot session, each `apply-now` replaces whatever hot layer was previously mounted at that scope rather than reconciling multiple applies into one — harmless for `usr`/`opt` specifically (no submounts to orphan there), but still not a merge; a reboot always reads `state.json`/generation symlinks fresh regardless of hot-apply history

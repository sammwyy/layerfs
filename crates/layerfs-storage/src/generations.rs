//! `update`/`update-head` are symlinks into `generations/`, swapped
...
/// Allocates a fresh, not-yet-existing path under `<store>/generations/`
/// for a new `prefix` generation; callers populate it themselves.
...
/// Atomically points the `<store>/<name>` symlink at `target` (build under
/// a temp name, then `rename(2)` over it — `<name>` is never briefly absent).
...
/// Reads what `<store>/<name>` currently points at, if anything.
